...
/*
    For when I come back and have no idea

    The id classification
    use the "tag-name"
    Current: div, btn, cord
...
for (let j = 0; j < Object.keys(thing).length; j++){
    if (Object.hasOwn(thing[Object.keys(thing)[j]], "cord")){
        for (let i = 0; i < thing[Object.keys(thing)[j]].cord.length; i++) {
            document.getElementsByClassName("row")[thing[Object.keys(thing)[j]].cord[i][1]].getElementsByClassName("col")[thing[Object.keys(thing)[j]].cord[i][0]].textContent = Object.keys(thing)[j];
        }
    } else {
        break;
    }
...
// failsafe
console.log("remember, the grid must be a perfect square")
temp_num = document.getElementsByClassName("row")[0].childElementCount;
for (let i = 1;i < document.getElementById("table_main").childElementCount;i++){
    if (temp_num == document.getElementsByClassName("row")[i].childElementCount){
        temp_num = document.getElementsByClassName("row")[i].childElementCount;
    } else{
        console.log("YOU FORGOT TO ADD A COLOUMN TO GRID/ THE GRID HAS TO BE PERFECT SQUARE")
    }
}
...
function update() {
    ...
    player.cord.x = player.cord.x > document.getElementsByClassName("row")[0].childElementCount-1 ? player.cord.x - 1 : player.cord.x;
    player.cord.x = player.cord.x == -1 ? player.cord.x + 1 : player.cord.x;

    player.cord.y = player.cord.y > document.getElementsByClassName("row")[0].childElementCount-1 ? player.cord.y - 1 : player.cord.y;
...
function collect_manual(){
    temp_string = document.getElementsByClassName("row")[player.cord.y].getElementsByClassName("col")[player.cord.x].textContent;
    ...
    //failsafe
    if (Object.hasOwn(thing[temp_string], "thing_resource") == false){
        console.log("NO THING_RESROUCE IN OBJECT");
        return;
    }
    ...
    // use for-loop & "string".includes("string") if many resources
